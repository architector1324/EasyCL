...
ecl::Error::Error(const char* prefix)
{
    this->prefix = prefix;
}
void ecl::Error::checkError(ecl::ErrorObject *obj) const
{
    if(obj->getErrorCode() < 0){
        std::string error = prefix;
        error += this->getError(obj->getErrorCode());
        throw std::runtime_error(error);
    }
}
void ecl::Error::checkSharedError() const
{
    if(SharedErrorObject::getSharedErrorCode() < 0){
        std::string error = prefix;
        error += this->getError(SharedErrorObject::getSharedErrorCode());
        throw std::runtime_error(error);
    }
}
...
void ecl::Platform::init(cl_device_type type)
{
...
ecl::CLError::CLError(const char* prefix) : Error(prefix) {}
...
ecl::GPGPUError::GPGPUError(const char* prefix) : Error(prefix) {}
...
ecl::GPU::GPU(size_t platform_index, size_t device_index)
{
    ...
    CreateContextError.checkError(this);

    queue = clCreateCommandQueue(context,* device, 0,& error_code);
    CreateQueueError.checkError(this);

    initialized = true;
}
void ecl::GPU::sendData(const std::vector<GPUArgument*>& args)
{
...
void ecl::GPU::receiveData(const std::vector<GPUArgument*>& args)
{
	for (size_t i(0); i < args.size(); i++) {
        args.at(i)->checkBuffer(&context);
        ...
        ReadBufferError.checkError(this);
	}
}
...
void ecl::GPUProgram::checkProgram(cl_context* context, cl_device_id* device)
{
    ...
    if(program.find(context) == program.end()){
        ...
        CreateProgramError.checkError(this);

        error_code = clBuildProgram(program.at(context), 0, nullptr, nullptr, nullptr, nullptr);
        if(error_code < 0) throw std::runtime_error(getBuildError(context, device));
    }
}
...
void ecl::GPUFunction::checkKernel(cl_program* program)
{
